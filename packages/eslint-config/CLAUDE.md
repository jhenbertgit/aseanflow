...
**@aseanflow/eslint-config** — Shared flat ESLint config (eslint@9+) for ASEANFlow monorepo. Uses `typescript-eslint`, `eslint-config-prettier`, framework-specific overrides for Next.js, NestJS, React internals. Exported as composable configs via package `exports` map.
...
- **`no-unused-vars` with `^_` ignore** — `_`-prefixed args OK
- **`no-explicit-any: warn`** — warn, not block
