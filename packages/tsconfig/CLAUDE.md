...
**@aseanflow/tsconfig** — Shared TS config presets for ASEANFlow monorepo. Base, Next.js, NestJS, React library configs. Consumed via `extends` in each package's `tsconfig.json`.
...
- **`noEmit: true`** — tsc for type-checking only; turbo handles builds
- **`isolatedModules: true`** — each file independent module, required by bundlers
- **`strict: true` + `strictNullChecks: true`** — no implicit any, no unchecked nulls

### Prisma v7 Note

`packages/database` uses base config. Prisma v7 generated client output (`generated/prisma/`) must be in `.gitignore` but accessible to TS. No special tsconfig changes needed — `moduleResolution: "bundler"` resolves generated output correctly.