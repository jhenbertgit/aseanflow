...
**@aseanflow/shared** — Zod schemas, TypeScript types, and pure utility functions shared across frontend (Next.js) and backend (NestJS). Single source of truth for API contracts: `CreateQuoteSchema`, `CreateTransferSchema`, `QuoteResponse`, `TransferDetailResponse`. No side effects, no framework dependencies — only `zod`.
...
- **Pure functions only** — no side effects, no state, no framework dependencies
- **No circular imports** — never import from other workspace packages
- **Zod schemas are the contract** — frontend and backend both derive types from the same schemas
- **No float for money** — response interfaces use `number` but backend must use `Prisma.Decimal` internally
- **No over-engineering** — hackathon MVP, only add schemas/types actually used by ASEANFlow transfer flow
- **Zod version**: `^3.23.8` — ensure consuming packages match

### Key Dependencies

- `zod@^3.23.8` — schema validation and type inference (only runtime dep)
