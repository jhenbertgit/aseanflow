...
**@aseanflow/redis** — `redis@^4.6.0` wrapper. Singleton client (`createRedisClient`, `getRedisClient`, `closeRedisClient`), `Cache` class w/ TTL + key prefix, `SessionManager` for sessions/refresh tokens, typed `RedisConfig`/`CacheOptions`. NestJS API uses: FX rate cache (TTL 30s), idempotency keys, BullMQ backend.
...
- **FX cache TTL = 30s** — PRP 02 requires 30s expiry
- **Singleton client** — one `createRedisClient` call, `getRedisClient` returns it everywhere
- **Key prefix** — default `webgenix:` (legacy). Override via `CacheOptions.prefix` to `aseanflow:`. TODO: update default.
- **BullMQ** — shares same Redis instance for job queues
- **Idempotency keys** — use `SET NX` pattern for transfer dedup
- **No float in cache** — serialize `Prisma.Decimal` as string before caching
