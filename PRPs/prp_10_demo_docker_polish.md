...
description: |
  ...
  Final polish PRP. Architecture demo page for judges, Docker Compose production config, seed data for demo stability, health check endpoint, UI polish.
  ...
  Build `/demo` page, production Docker Compose (web, api, worker, postgres, redis), `GET /api/health`, seed data with sample completed transfer, polish UI animations + responsive layouts.

  ## Why

  - **Judge-facing**: Architecture diagram shows technical depth
  - **Deployable**: Docker Compose = one-command demo
  - **Demo stability**: Seed data works even if live services hiccup
  ...
  - `GET /api/health` returns postgres, redis, morph status
