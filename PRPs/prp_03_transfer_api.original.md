...
description: |
  ...
  Build the core Transfer API. NestJS controller + service handling quote generation, transfer creation with tracking code, and transfer lookup by tracking code.
  ...
  Create NestJS TransferModule with controller exposing 3 endpoints: POST /api/quote, POST /api/transfer, GET /api/transfer/:trackingCode. Service handles transfer lifecycle, state machine, and tracking code generation.

  ## Why

  - **Core API surface**: These 3 endpoints power the entire frontend experience
  - **State machine foundation**: Transfer status transitions must be correct for settlement flow
  - **Tracking code**: Primary user-facing identifier — no login means this IS the auth

  ## What

  ### User-visible behavior
  - POST /api/quote → instant FX quote with rate, fee, receiveAmount
  - POST /api/transfer → creates transfer, returns tracking code
  - GET /api/transfer/:trackingCode → returns full transfer detail with status

  ### Technical requirements
  - NestJS module: TransferModule importing FxModule
  - NestJS controller: TransferController with 3 endpoints
  - NestJS service: TransferService with createTransfer, advanceStatus, getByTrackingCode
  - State machine: TransferStatus transitions are linear, never skip, never go back
  - Idempotency: Redis key `transfer:idempotency:{key}` TTL 60s to prevent duplicates
  - Tracking code format: `TXN` + random alphanumeric, 12 chars

  ### Success Criteria

  - [ ] POST /api/quote returns { rate, fee, receiveAmount, timestamp }
  - [ ] POST /api/transfer creates transfer with status CREATED
  - [ ] POST /api/transfer returns { trackingCode, status }
  - [ ] Tracking code is unique and 12+ chars starting with TXN
  - [ ] GET /api/transfer/:trackingCode returns full transfer detail
  - [ ] State machine never allows skipping or reversing states
  - [ ] Idempotency key prevents duplicate transfers
  - [ ] DTOs validated with class-validator + Zod
  - [ ] Domain events emitted on status change
  ...
  - [ ] POST /api/quote returns correct quote
  - [ ] POST /api/transfer creates with tracking code
  - [ ] GET /api/transfer/:trackingCode returns detail
  - [ ] State machine enforces linear transitions
  ...
  - Do NOT add authentication — tracking code IS the identifier
