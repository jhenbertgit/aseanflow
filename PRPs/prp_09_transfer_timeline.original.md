...
description: |
  ...
  Build the transfer timeline page at /transfer/[id]. Polls transfer status every 1s via React Query. Animated Framer Motion timeline showing all 7 states. Morph proof badge appears on MORPH_ANCHORED.
  ...
  Create /transfer/[trackingCode] page with: animated timeline showing 7 transfer states, polling GET /api/transfer/:trackingCode every 1s via React Query, Framer Motion AnimatePresence for state transitions, Morph proof component that appears on MORPH_ANCHORED status.

  ## Why

  - **Demo centerpiece**: The animated timeline IS the demo — judges watch the transfer progress
  - **Morph showcase**: Morph proof is the payoff — visible after settlement
  - **Real-time feel**: Polling + animations make it feel live

  ## What

  ### User-visible behavior
  - See animated timeline with 7 states
  ...
  - [ ] Timeline shows all 7 states with correct labels
  - [ ] States animate as they progress (AnimatePresence)
  - [ ] Polling starts at 1s interval
  - [ ] Polling stops when MORPH_ANCHORED reached
