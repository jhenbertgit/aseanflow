...
description: |
  ...
  Build the ASEANFlow landing page. Hero section with tagline, CTA buttons, tech badges. Framer Motion entrance animations. Route: `/` (apps/web/app/page.tsx).
  ...
  Create the landing page at `/` with: hero section ("ASEANFlow — SWIFT-Free ASEAN Payments"), subtitle ("Send PHP → IDR directly without SWIFT or USD"), "Send Money" button → /send, "View Architecture" button → /demo, tech badges (Morph, NestJS, Docker, Redis). Framer Motion entrance animations.

  ## Why

  - **First impression**: Judges see this first — must convey value in 10 seconds
  - **Clear CTA**: "Send Money" immediately starts the demo flow
  - **Tech credibility**: Badges show the stack without over-explaining
  ...
  - "Send Money" button navigates to /send
  - "View Architecture" button navigates to /demo
  ...
  - Framer Motion for entrance animations (AnimatePresence, motion.div)
  ...
  - Do NOT over-animate — subtle entrance effects only
  - Do NOT add authentication or login prompts
  - Do NOT fetch data on landing page — static content
  - Do NOT use heavy images — keep load fast
  - Do NOT use tailwind.config.js — Tailwind v4 uses CSS config
