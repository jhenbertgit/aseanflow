...
description: |
  ...
  Build `/send` page. PHP amount input → live IDR conversion (React Query) → "Continue Transfer" creates transfer, redirects to timeline.
  ...
  Create `/send` page: PHP amount input, live IDR conversion (React Query → `POST /api/quote`), fee display (₱10), ETA (~10 sec), "Continue Transfer" → `POST /api/transfer` → redirect `/transfer/[trackingCode]`.

  ## Why

  - **Core user flow**: Payment experience starts here
  - **Live feedback**: Real-time conversion as user types
  - **React Query pattern**: Proper data fetching + caching

  ## What

  ### User-visible behavior
  - Enter PHP amount → IDR receive amount updates
  - See fee (₱10) + ETA (~10 sec)
  - "Continue Transfer" → redirect to timeline
  ...
  - [x] PHP amount input accepts numeric values
  - [x] IDR conversion updates as user types (debounced)
  - [x] Fee ₱10 displayed
  - [x] ETA ~10 sec displayed
  - [x] "Continue Transfer" creates transfer via API
  - [x] Redirects to `/transfer/[trackingCode]` on success
  - [x] Error handling for invalid amounts
  - [x] React Query for API calls with cache/loading states
