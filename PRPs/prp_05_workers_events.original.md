name: "PRP 05 — Workers & Events: BullMQ Processors + Domain Events"
description: |

  ## Purpose

  Set up BullMQ worker processes for settlement and morph-anchor jobs. Define domain events using NestJS EventEmitter2. Workers run as separate process — NOT in the NestJS API process.
  ...
  Create standalone BullMQ worker app that processes settlement and morph-anchor jobs. Define domain events (transfer.status.changed, transfer.settled) using NestJS EventEmitter2. Wire settlement queue into the transfer creation flow.

  ## Why

  - **Critical architecture**: BullMQ workers MUST be separate process from NestJS API
  - **Async Morph anchoring**: Settlement completes → queues morph-anchor → worker picks it up
  - **Decoupling**: Events allow UI updates and Morph anchoring without blocking settlement
  ...
  - Morph anchoring happens asynchronously after settlement completes
  ...
  - [ ] Settlement queue processes transfer through all states
  - [ ] Morph-anchor queue processes proof after SETTLED
  - [ ] Worker runs as separate process (not in API process)
  - [ ] Domain events emitted on status changes
  - [ ] Transfer controller queues settlement on create
  ...
  - [ ] Worker starts without errors: `cd apps/worker && pnpm start`
  - [ ] Settlement queue processes jobs
  - [ ] Morph-anchor queue processes jobs after SETTLED
