...
description: |
  ...
  Build FX rate engine. Cached exchange rates (Redis, TTL 30s) + quote calculation. Consumed by Quote API (PRP 03).
  ...
  NestJS FxModule + FxService: exchange rate lookup (Redis-cached, 30s TTL) + quote calc (rate + fee + receiveAmount). Hardcoded rate 289.2 PHP/IDR for hackathon.

  ## Why

  - **Core business logic**: Every transfer needs quote — FX engine = heart of system
  - **Redis integration**: Caching pattern, prevents stale rates
  - **Foundation for Quote API**: PRP 03 consumes this service

  ## What

  ### User-visible behavior
  - None — internal service consumed by API

  ### Technical requirements
  - NestJS module: FxModule
  - NestJS service: FxService with `getRate()` and `calculateQuote()`
  - Redis cache key pattern: `fx:{from}:{to}`, TTL 30s
  - Fallback rate: 289.2 PHP/IDR (hardcoded hackathon)
  - Fee: flat ₱10, deducted before conversion
  - DTO with class-validator for input validation

  ### Success Criteria

  - [x] FxModule registers in NestJS without errors
  - [x] `getRate('PHP', 'IDR')` returns 289.2 on cache miss
  - [x] `getRate('PHP', 'IDR')` returns cached value within TTL
  - [x] `calculateQuote(1000, 'PHP', 'IDR')` returns rate 289.2, fee 10, receiveAmount 286308 (fee deducted before conversion)
  - [x] Redis cache key is `fx:PHP:IDR` with TTL 30s
  - [x] DTO validates amount positive + within range
  - [x] Unit tests pass for rate caching + quote calculation
  ...
  - [x] FxModule registers without errors
  - [x] Unit tests pass: `cd apps/api && pnpm test -- --verbose`
  - [x] No linting errors: `pnpm lint`
  - [x] Redis cache pattern correct: `fx:{from}:{to}` with TTL 30s

  ---

  ## Anti-Patterns to Avoid

  - Do NOT use float for stored money — FX service returns numbers for quotes only
  - Do NOT make external API calls — hardcoded rate for hackathon
  - Do NOT set TTL > 30s — stale rates cause inaccurate quotes
  - Do NOT deduct fee after conversion — fee deducted before FX conversion
