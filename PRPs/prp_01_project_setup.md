...
description: |
  ...
  Foundation PRP. DB schema (Transfer, LedgerEntry), shared TS types/Zod schemas, Docker Compose (PostgreSQL + Redis). All PRPs depend on this.

  ## Core Principles

  1. **Context is King**: Include ALL docs, examples, caveats
  2. **Validation Loops**: Executable tests/lints AI can run and fix
  3. **Information Dense**: Keywords and patterns from codebase
  ...
  Data layer + infra foundation for ASEANFlow. Prisma schema (Transfer, LedgerEntry), shared TS types with Zod validation, Docker Compose (PostgreSQL + Redis).

  ## Why

  - **Foundation for all PRPs**: Every backend/frontend module depends on these types and models
  - **Type safety**: Shared types keep frontend and backend in sync
  - **Infrastructure**: PostgreSQL + Redis must run before backend code works
  ...
  - Prisma schema: Transfer, LedgerEntry, Currency enum, TransferStatus enum
  - Shared types package with Zod schemas (Quote, Transfer DTOs)
  - Docker Compose with PostgreSQL and Redis services
  - Prisma client generated, database synced

  ### Success Criteria

  - [x] Prisma schema has Transfer model with all fields from blueprint
  - [x] Prisma schema has LedgerEntry model linked to Transfer
  - [x] Currency enum has PHP and IDR
  - [x] TransferStatus enum has all 7 states in correct order
  - [ ] `pnpm db:generate` succeeds without errors
  - [ ] `pnpm db:sync` succeeds (Docker running)
  - [x] Shared types export CreateQuoteSchema, CreateTransferSchema
  - [x] Shared types export QuoteResponse, TransferResponse, TransferDetailResponse
  - [x] Docker Compose has postgres and redis services
  ...
  - [x] Prisma schema compiles: `pnpm db:generate`
  - [ ] Database syncs: `pnpm db:sync`
  - [x] Shared types importable: no TypeScript errors
  - [x] Docker services running: `docker-compose -f docker-compose.dev.yml ps`
