...
description: |
  ...
  Build settlement simulators (InstaPay for PHP, BI-FAST for IDR), settlement orchestration service, and double-entry ledger. Simulates realistic domestic payment rail processing with 1000-1500ms delays.
  ...
  Create InstaPay and BI-FAST simulators with realistic delays, SettlementService that orchestrates the full settlement flow through all states, and LedgerService for double-entry bookkeeping.

  ## Why

  - **Core demo value**: Simulated rails show the "SWIFT-free" concept in action
  - **State machine orchestration**: Settlement drives the transfer through all statuses
  - **Audit trail**: Double-entry ledger provides financial integrity proof

  ## What

  ### User-visible behavior
  - Transfer progresses through animated states: QUOTE_LOCKED → INSTA_PAY_PROCESSING → FX_CONVERSION → BI_FAST_PROCESSING → SETTLED
  ...
  - SettlementService: orchestrates full flow, advances state at each step, queues morph-anchor after SETTLED
  - LedgerService: creates debit (PHP) and credit (IDR) entries per transfer

  ### Success Criteria

  - [ ] InstaPay simulator completes in 1000-1500ms
  - [ ] BI-FAST simulator completes in 1000-1500ms
  - [ ] SettlementService advances through all states in correct order
  - [ ] instapayRef and bifastRef stored on transfer record
  - [ ] LedgerService creates debit and credit entries
  - [ ] Morph-anchor job queued after SETTLED (not awaited)
  - [ ] Events emitted at each status change
  - [ ] Unit tests pass for simulators and orchestration
  ...
  - [ ] Simulators delay 1000-1500ms (not instant)
  - [ ] Orchestration advances linearly through all states
  - [ ] Morph-anchor queued (not awaited) after SETTLED
  - [ ] Ledger creates paired debit/credit entries
