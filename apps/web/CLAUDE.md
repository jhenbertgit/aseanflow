Here's compressed output:

# CLAUDE.md - @aseanflow/web

## FEATURE:

**ASEANFlow Frontend** — Next.js 16 frontend. SWIFT-free cross-border PHP→IDR payments. 5 screens: landing `/`, quote `/send`, transfer timeline `/transfer/[id]`, demo `/demo`, health API proxy. No auth. Tracking code = sole identifier. Framer Motion animations, React Query polling, Morph proof display.
...
- **No auth/login/register** — no authentication. Tracking code = ONLY identifier
- **Tailwind v4** uses CSS-based config — NOT `tailwind.config.js`
- **shadcn/ui** added via `pnpm dlx shadcn@latest add <component>` from `apps/web`
- **No float for money** — use backend Decimal values, format with `Intl.NumberFormat`
...
- **Morph proof** — async, never block UI. Show proof hash only after `MORPH_ANCHORED`
...
- No Redux/global state stores — React Query for server state
...
cd apps/web && pnpm type-check  # type check
pnpm lint                   # lint all
```
...
Main changes: trimmed filler phrases ("that define this frontend", "display amounts using backend Decimal values"), merged redundant wording ("is the only identifier" → "= sole identifier"), condensed anti-pattern section headers, kept all code blocks/URLs/paths verbatim.